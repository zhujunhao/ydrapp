...
class SearchByCommunity extends Component {
    constructor(props) {
        super(props);
    ...
    loadData = async (loadMore, searchVal) => {
        const { searchinfo, searchCommunitys } = this.props;
        let nowstart;
        if (loadMore) {
        ...
        let result = await get(url.searchCommunityList(encodeURI(searchVal),"01", nowstart, this.state.count))
        if (result.status == 200 && result.data) {
            ...
            if (loadMore) {
                let orginArr = searchinfo.searchcommunitys;
    ...
    onRightButtonClick = async (searchVal) => {
        ...
        if (searchinfo.showText === '搜索') {
            let keyval = await  AsyncStorage.getItem('communityArr')
            if (keyval && keyval!=undefined) {
                //排重
                if (keyval.indexOf(searchinfo.inputKey) == -1) {
                    keyval = keyval + ',' + searchinfo.inputKey
                    AsyncStorage.setItem('communityArr',keyval)
                }
            } else {
                AsyncStorage.setItem('communityArr',searchinfo.inputKey)
            }
    ...
    changetext(inputtext) {
        const { searchCommunitys } = this.props;
        searchCommunitys({
            retCode: "0003",
            inputKey: inputtext
        })
    ...
    renderNavBar() {
        ...
        const placeholder = inputKey || "请输入关键字";
    ...
    historyView = async () => {
        let result = await AsyncStorage.getItem('communityArr')
        if (result) {
            this.setState({
                historyStr: result
            })
    ...
    render() {
        ...
        let listView = <FlatList
                data={searchinfo.searchcommunitys}
        ...
        return <View style={{flex:1,backgroundColor:'#f5f5f5'}}>
            {navigationBar}
            {this.renderNavBar()}
            {searchinfo.searchcommunitys && searchinfo.searchcommunitys.length>0? listView : historyViews}
            <Toast ref={'toast'}
                position={'center'}
            />
        </View>
...
const mapDispatchToProps = dispatch => ({
    searchCommunitys: (sendInfo) => dispatch(actions.searchCommunitys(sendInfo)),
});

//注意：connect只是个function，并不应定非要放在export后面
export default connect(mapStateToProps, mapDispatchToProps)(SearchByCommunity)
